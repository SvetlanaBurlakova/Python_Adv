...
def menu(balance: Decimal, count: int, isFlag: bool):
...
def get_money(balance: Decimal):
    ...
    if money_to_get % MIN_SUM == 0:
        ...
        if money_to_get + procent <= balance:
            money = balance - (money_to_get + procent)
            return money
        else:
...
if __name__ == '__main__':
    ...
    isFlag = True
    while isFlag:
        balance, isFlag = menu(balance, count, isFlag)
